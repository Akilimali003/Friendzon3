...
//        val reference = FirebaseDatabase.getInstance().getReference("/messages").push()
        ...
        val chatMessage = ChatMessage(reference.key!!, text, fromId, toId, System
            .currentTimeMillis() / 1000) //the timestamp return the value in seconds

        reference.setValue(chatMessage)
            .addOnSuccessListener {
                Log.d(TAG, "Messages save: ${reference.key}")
...
class ChatFromItem(val text: String, val user: User): Item<ViewHolder>(){
    ...
    override fun bind(viewHolder: ViewHolder, position: Int) {
        ...
        //load user image into the right star
...
class ChatToItem(val text: String, val user: User): Item<ViewHolder>(){
    ...
    override fun bind(viewHolder: ViewHolder, position: Int) {
        ...
        //load user image into the left star
        val uri = user.profileImageUrl
